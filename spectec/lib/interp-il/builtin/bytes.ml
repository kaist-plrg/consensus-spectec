...
let validate_bytes32 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let bytes32_max = Bigint.(pow (of_int 2) (of_int 256)) in
  if Bigint.(x >= zero && x < bytes32_max) then Ok ()
  else Error (Err.runtime at "bytes32: value must be in range [0, 2^256)")

let validate_bytes31 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let bytes31_max = Bigint.(pow (of_int 2) (of_int 248)) in
  if Bigint.(x >= zero && x < bytes31_max) then Ok ()
  else Error (Err.runtime at "bytes31: value must be in range [0, 2^248)")

let validate_bytes28 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let bytes28_max = Bigint.(pow (of_int 2) (of_int 224)) in
  if Bigint.(x >= zero && x < bytes28_max) then Ok ()
  else Error (Err.runtime at "bytes28: value must be in range [0, 2^224)")

let validate_bytes20 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let bytes20_max = Bigint.(pow (of_int 2) (of_int 160)) in
  if Bigint.(x >= zero && x < bytes20_max) then Ok ()
  else Error (Err.runtime at "bytes20: value must be in range [0, 2^160)")

let validate_bytes4 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let bytes4_max = Bigint.(pow (of_int 2) (of_int 32)) in
  if Bigint.(x >= zero && x < bytes4_max) then Ok ()
  else Error (Err.runtime at "bytes4: value must be in range [0, 2^32)")

let validate_bytes1 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let bytes1_max = Bigint.(pow (of_int 2) (of_int 8)) in
  if Bigint.(x >= zero && x < bytes1_max) then Ok ()
  else Error (Err.runtime at "bytes1: value must be in range [0, 2^8)")

let validate_uint8 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let uint8_max = Bigint.(pow (of_int 2) (of_int 8)) in
  if Bigint.(x >= zero && x < uint8_max) then Ok ()
  else Error (Err.runtime at "uint8: value must be in range [0, 2^8)")

let validate_uint32 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let uint32_max = Bigint.(pow (of_int 2) (of_int 32)) in
  if Bigint.(x >= zero && x < uint32_max) then Ok ()
  else Error (Err.runtime at "uint32: value must be in range [0, 2^32)")

let validate_uint64 (at : Util.Source.region) (x : Bigint.t) :
    (unit, Err.t) result =
  let uint64_max = Bigint.(pow (of_int 2) (of_int 64)) in
  if Bigint.(x >= zero && x < uint64_max) then Ok ()
  else Error (Err.runtime at "uint64: value must be in range [0, 2^64)")
...
let uint_to_bytes ~at (uint_val : Num.t) : (Value.t, Err.t) result =
  ...
  then
    ...
    Error
      (Err.runtime at "uint_to_bytes: value too large for uint8/uint32/uint64")
...
let concat_bytes ~at (value_a : Value.t) (value_b : Value.t) :
  ...
  | _ -> Error (Err.runtime at "concat_bytes: expected bytes values")
...
let bytes32_to_bytes ~at (value : Value.t) : (Value.t, Err.t) result =
  ...
  | NumV n ->
      ...
      let* () = validate_bytes32 at bytes32_val in
      Ok (make_bytes ~num:bytes32_val ~len:32)
  | _ -> Error (Err.runtime at "bytes32_to_bytes: expected bytes32 or NumV")
...
let bytes4_to_bytes ~at (value : Value.t) : (Value.t, Err.t) result =
  ...
  | NumV n ->
      ...
      let* () = validate_bytes4 at bytes4_val in
      Ok (make_bytes ~num:bytes4_val ~len:4)
  | _ -> Error (Err.runtime at "bytes4_to_bytes: expected bytes4 or NumV")
...
let extract_execution_address ~at (bytes32_val : Num.t) :
    (Value.t, Err.t) result =
  let bytes32_val = Num.to_int bytes32_val in
  at |> ignore;
  let* () = validate_bytes32 at bytes32_val in
...
let shr_uint64 ~at (x : Num.t) (k : Num.t) : (Value.t, Err.t) result =
  ...
  if Bigint.compare k Bigint.zero < 0 then
    ...
    try
      ...
      else Error (Err.runtime at "shr_uint64: shift amount must be in [0, 64]")
    with _ -> Error (Err.runtime at "shr_uint64: shift amount too large")
