...
let to_set_ ~at (typ : targ) (vs : Value.t list) : (Value.t, Err.t) result =
  ...
  let result =
    List.fold_left
      (fun acc v ->
        if Sets.VSet.mem v !seen then acc
        else (
...
let repeat_ ~at (typ : targ) (value : Value.t) (count : Num.t) :
    (Value.t, Err.t) result =
  let count = Num.to_int count in
  at |> ignore;
  try
    let n = Bigint.to_int_exn count in
    if n < 0 then Error (Err.runtime at "repeat: count must be non-negative")
    else
      let repeated_list = List.init n (fun _ -> value) in
      Ok (Value.list typ repeated_list)
  with _ ->
    Error (Err.runtime at "repeat: count is too large to be an integer")
...
let range ~at (count : Num.t) : (Value.t, Err.t) result =
  ...
  try
    let n = Bigint.to_int_exn count in
    if n < 0 then Error (Err.runtime at "range: count must be non-negative")
    else
      let range_list = List.init n (fun i -> Value.nat (Bigint.of_int i)) in
      Ok (Value.list' Typ.int range_list)
  with _ ->
    Error (Err.runtime at "range: count is too large to be an integer")
...
let set_or_append_list_ ~at (typ : targ) (vs : Value.t list) (idx_big : Num.t)
    (value : Value.t) : (Value.t, Err.t) result =
  ...
  try
    ...
    if idx < 0 then
      ...
      else Error (Err.runtime at "set_or_append_list: index out of range")
  with _ ->
    Error
      (Err.runtime at "set_or_append_list: index is too large to be an integer")
