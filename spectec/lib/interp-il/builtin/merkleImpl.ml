...
let hash_tree_root_beaconBlockHeader ~at (hdr : Value.t) : (Value.t, Err.t) result =
  let get_nat v = v |> Runtime_dynamic.Value.get_num |> Num.to_int in
...
let hash_tree_root_depositData ~at (dd : Value.t) : (Value.t, Err.t) result =
  let get_num v = v |> Runtime_dynamic.Value.get_num |> Num.to_int in
...
let hash_tree_root_eth1Data ~at (v : Value.t) : (Value.t, Err.t) result =
  let get_nat x = x |> Runtime_dynamic.Value.get_num |> Num.to_int in
...
let hash_tree_root_executionPayload ~at (v : Value.t) : (Value.t, Err.t) result =
  let get_nat v = v |> Runtime_dynamic.Value.get_num |> Num.to_int in
  let get_bytes_len v = v |> Runtime_dynamic.Value.get_bytes in
  (* 15 fields *)
  let* temp =
    match v.it with
  ...
  let tx_elem_root tx =
    ...
    let data_bytes = be_of_bigint_fixed data ~len:len in
  ...
  let r_withdrawals = match r_withdrawals_v.it with
...
let hash_tree_root_SignedBLSToExecutionChange ~at (v : Value.t) : (Value.t, Err.t) result =
  ...
  let msg_root = match msg_root_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
...
let hash_tree_root_Deposit ~at (v : Value.t) : (Value.t, Err.t) result =
  ...
  let r_data = match r_data_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
...
let hash_tree_root_AttestationData ~at (v : Value.t) : (Value.t, Err.t) result =
  ...
  let r_source = match r_source_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
  let* r_target_v = hash_tree_root_Checkpoint ~at target in
  let r_target = match r_target_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
...
let hash_tree_root_AttesterSlashing ~at (v : Value.t) : (Value.t, Err.t) result =
  ...
  let r_a1 = match r_a1_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
  let* r_a2_v = hash_tree_root_IndexedAttestation ~at attestation_2 in
  let r_a2 = match r_a2_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
...
let hash_tree_root_SignedBeaconBlockHeader ~at (v : Value.t) : (Value.t, Err.t) result =
  ...
  let r_message = match msg_root_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
  (* signature: BLSSignature(96) → ByteVector[96] *)
  let* () = ensure_fits_bytes ~at (get_nat signature) ~len:96 in
  let sig_bytes = be_of_bigint_fixed (get_nat signature) ~len:96 in
...
let hash_tree_root_ProposerSlashing ~at (v : Value.t) : (Value.t, Err.t) result =
  ...
  let r_sh1 = match r_sh1_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
  let* r_sh2_v = hash_tree_root_SignedBeaconBlockHeader ~at signed_header_2 in
  let r_sh2 = match r_sh2_v.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
...
let hash_tree_root_beaconBlockBody ~at (v : Value.t) : (Value.t, Err.t) result =
  ...
  let to_b32 (rv: Value.t) : Bytes.t = match rv.it with NumV n -> leaf_bytes32 (Num.to_int n) | _ -> zero32 in
  ...
  let list_htr (xs: Value.t list) (f: Value.t -> (Value.t, Err.t) result) : (Bytes.t, Err.t) result =
    let rec mapM acc = function
      | [] -> Ok (List.rev acc)
      | x::tl -> let* rv = f x in mapM (to_b32 rv :: acc) tl
    in
...
let builtins : (string * Define.t) list =
  [
    ("is_valid_merkle_branch",
      Define.T0.a5 Arg.nat (Arg.list_of Arg.nat) Arg.nat Arg.nat Arg.nat is_valid_merkle_branch);
    ("hash_tree_root_roots",
      Define.T0.a1 (Arg.list_of Arg.nat) hash_tree_root_roots);
    ("hash_tree_root_tx",
      Define.T0.a1 (Arg.list_of Arg.nat) hash_tree_root_tx);
