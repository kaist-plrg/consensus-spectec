...
let validate_fits_len ~at (x : Bigint.t) (len : int) : (unit, Err.t) result =
  let open Bigint in
  let pow2_8 n =
  ...
  if x >= zero && x < pow2_8 len then Ok ()
    ...
    Error
      (Err.runtime at
         (Printf.sprintf "hash_<X>: value does not fit %d bytes" len))
  ...
  let* num, len =
    ...
    | NumV _ -> (
        ...
        | None ->
            ...
            Error
              (Err.runtime at "hash_<X>: cannot infer byte length from type"))
