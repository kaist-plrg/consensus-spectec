...
let bitacc ~at (n : Bigint.t) (m : Bigint.t) (l : Bigint.t) : Value.t result =
  try
    if Bigint.(l < zero) then
      Error (runtime at "bitacc: slice x[y:z] must have z >= 0")
    else if Bigint.(m < l) then
      Error (runtime at "bitacc: slice x[y:z] must have y >= z")
    else
  ...
  | _ -> Error (runtime at "bitacc: unexpected error during calculation")
