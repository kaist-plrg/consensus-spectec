...
let outcome_color = function
  ...
  | Task.Skipped -> Diagnostic.Ansi.Yellow
...
let print_summary summary =
  ...
  in
  Format.printf "\nTest Results: %d/%d passed, %d failed%s\n" passed
...
let run_target ?(config = Instrumentation.Config.default) ?test_dir ?skip_source
    ~ansi ~(checkpoint_config : Checkpoint.config) ~verbose ~mode ~spec_files
    spec_il tasks =
  Instrumentation.with_instrumentation config
    (Instrumentation.Static.IlSpec spec_il)
  @@ fun () ->
            ...
            (fun index input ->
              ...
              if verbose then
                Format.printf "  [%d/%d] %s... %!"
                  ...
                  total_all source;
              let result =
                match skip_source with
                | Some skip when skip source ->
                    if verbose then print_outcome_tag ansi Task.Skipped;
