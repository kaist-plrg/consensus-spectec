...
module type S = sig
  ...
  val on_prem_enter :
    values:(Il.exp * Il.Value.t) list -> prem:Il.prem -> at:region -> unit
  ...
  (* Readset provenance for a call's result, written into the handler's side
     table keyed by [result]'s vid. Annotation-only. [lookup_clauses] also
