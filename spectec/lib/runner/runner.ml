            ...
            ~coverage:
              {
                ...
                dependency =
                  Some (Instrumentation.Dependency.Positive.get_result ());
                path_condition =
                  Some (Instrumentation.Dependency.Negative.get_result ());
              }
        in
  ...
  let parse_json () =
    ...
    let* value_il =
      ...
      |> Result.map_error (fun err ->
             ...
             Error.JsonParseError (Common.Source.no_region, msg))
    in
    Ok value_il
  in
