...
let hash_tree_root_beaconBlockHeader ~at (hdr : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
    ...
    | ListV [ slot_v; proposer_v; parent_v; state_v; body_v ] ->
        Ok
          ( get_nat slot_v,
            ...
            get_nat state_v,
            get_nat body_v )
    | _ -> Error (runtime at "beaconBlockHeader: unexpected value shape")
  in
...
let hash_tree_root_depositData ~at (dd : Value.t) : Value.t result =
  ...
  let* pubkey_b48, wcred_b32, amount_u64, sig_b96 =
    ...
    | ListV [ v_pub; v_wcr; v_amt; v_sig ] ->
        Ok (get_num v_pub, get_num v_wcr, get_num v_amt, get_num v_sig)
    | _ -> Error (runtime at "depositData: unexpected value shape")
  in
  (* range checks *)
...
let hash_tree_root_forkdata ~at (fd : Value.t) : Value.t result =
  let get_num v = v |> Value.get_num |> Num.to_int in
  let* version_b4, gvr_b32 =
    ...
    | _ -> Error (runtime at "forkdata: unexpected value shape")
  in
...
let htr_withdrawal_container ~at (w : Value.t) : Bytes.t result =
  let get_num v = v |> Value.get_num |> Num.to_int in
    ...
    | ListV [ v_index; v_validator_index; v_address; v_amount ] ->
        Ok
          ( get_num v_index,
            ...
            get_num v_address,
            get_num v_amount )
    | _ -> Error (runtime at "withdrawal: unexpected value shape")
  in
...
let hash_tree_root_eth1Data ~at (v : Value.t) : Value.t result =
  ...
  let* deposit_root_b32, deposit_count_u64, block_hash_b32 =
    ...
    | _ -> Error (runtime at "eth1Data: unexpected shape")
  in
...
let hash_tree_root_executionPayload ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  ...
  let* ( parent_hash,
           ...
           (_, v15);
              ...
              Ok
                ( v1,
                  ...
                  None,
                  None )
          | _ -> Error (runtime at "executionPayload: unexpected field count")
           ...
           (_, v17);
              ...
              Ok
                ( v1,
                  ...
                  Some v17 )
          | _ -> Error (runtime at "executionPayload: unexpected field count")
        else Error (runtime at "executionPayload: unexpected field count")
    | TupleV fields ->
        let count = List.length fields in
        if count = 15 then
          match fields with
    ...
    | _ -> Error (runtime at "executionPayload: unexpected value shape")
  in
...
let hash_tree_root_BLSToExecutionChange ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
    ...
    | _ -> Error (runtime at "BLSToExecutionChange: unexpected value shape")
  in
...
let hash_tree_root_SignedBLSToExecutionChange ~at (v : Value.t) : Value.t result
    =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
    ...
    | ListV [ v_msg; v_sig ] ->
        Ok (v_msg, v_sig)
    | _ ->
        Error (runtime at "SignedBLSToExecutionChange: unexpected value shape")
  in
...
let hash_tree_root_SyncAggregate ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
...
let hash_tree_root_VoluntaryExit ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
    ...
    | _ -> Error (runtime at "VoluntaryExit: unexpected value shape")
  in
...
let hash_tree_root_SignedVoluntaryExit ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
    | StructV [ (_, v_msg); (_, v_sig) ]
    ...
    | ListV [ v_msg; v_sig ] ->
        Ok (v_msg, v_sig)
    | _ -> Error (runtime at "SignedVoluntaryExit: unexpected value shape")
  in
  let message, signature = temp in
...
let hash_tree_root_Deposit ~at (v : Value.t) : Value.t result =
  let get_nat x = x |> Value.get_num |> Num.to_int in
  let get_list v = match v.it with ListV xs -> xs | _ -> [] in
  let* temp =
    match v.it with
    ...
    | ListV [ v_proof; v_data ] ->
        Ok (v_proof, v_data)
    | _ -> Error (runtime at "Deposit: unexpected value shape")
  in
...
let hash_tree_root_Checkpoint ~at (v : Value.t) : Value.t result =
  let get_nat x = x |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
    ...
    | ListV [ v_epoch; v_root ] ->
        Ok (v_epoch, v_root)
    | _ -> Error (runtime at "Checkpoint: unexpected value shape")
  in
...
let hash_tree_root_AttestationData ~at (v : Value.t) : Value.t result =
  let get_nat x = x |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
    | StructV
    ...
    | _ -> Error (runtime at "AttestationData: unexpected value shape")
  in
...
let hash_tree_root_Attestation ~at (v : Value.t) : Value.t result =
  let get_nat x = x |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
...
let hash_tree_root_IndexedAttestation ~at (v : Value.t) : Value.t result =
  let get_nat x = x |> Value.get_num |> Num.to_int in
  let get_list v = match v.it with ListV xs -> xs | _ -> [] in
  let* temp =
    match v.it with
...
let hash_tree_root_AttesterSlashing ~at (v : Value.t) : Value.t result =
  let* temp =
    match v.it with
    | StructV [ (_, a1); (_, a2) ] | TupleV [ a1; a2 ] | ListV [ a1; a2 ] ->
        Ok (a1, a2)
    | _ -> Error (runtime at "AttesterSlashing: unexpected value shape")
  in
...
let hash_tree_root_SignedBeaconBlockHeader ~at (v : Value.t) : Value.t result =
  let get_nat x = x |> Value.get_num |> Num.to_int in
  let* temp =
    match v.it with
    | StructV [ (_, v_msg); (_, v_sig) ]
    ...
    | ListV [ v_msg; v_sig ] ->
        Ok (v_msg, v_sig)
    | _ -> Error (runtime at "SignedBeaconBlockHeader: unexpected value shape")
  in
  let message, signature = temp in
...
let hash_tree_root_ProposerSlashing ~at (v : Value.t) : Value.t result =
  let* temp =
    match v.it with
    | StructV [ (_, sh1); (_, sh2) ] | TupleV [ sh1; sh2 ] | ListV [ sh1; sh2 ]
      ->
        Ok (sh1, sh2)
    | _ -> Error (runtime at "ProposerSlashing: unexpected value shape")
  in
...
let hash_tree_root_Fork ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let* previous_version, current_version, epoch =
    ...
    | _ -> Error (runtime at "Fork: unexpected value shape")
  in
...
let hash_tree_root_Validator ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let* ( pubkey,
    ...
    | _ -> Error (runtime at "Validator: unexpected value shape")
  in
...
let hash_tree_root_SyncCommittee ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
...
let hash_tree_root_ExecutionPayloadHeader ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  (* Detect fork by field count: 15 fields = Capella, 17 fields = Deneb *)
  let* ( parent_hash,
         fee_recipient,
    ...
    | StructV fields ->
        ...
        if count = 15 then
          ...
          | _ ->
              Error
                (runtime at "ExecutionPayloadHeader: unexpected field count")
        else if count = 17 then
          match fields with
          | [
           (_, v1);
          ...
          | _ ->
              Error
                (runtime at "ExecutionPayloadHeader: unexpected field count")
        else Error (runtime at "ExecutionPayloadHeader: unexpected field count")
    | TupleV fields ->
        let count = List.length fields in
        if count = 15 then
          match fields with
    ...
    | _ -> Error (runtime at "ExecutionPayloadHeader: unexpected value shape")
  in
  (* 1. parent_hash: bytes32 *)
  let* () = ensure_fits_bytes ~at (get_nat parent_hash) ~len:32 in
  let r_parent_hash = leaf_bytes32 (get_nat parent_hash) in
...
let hash_tree_root_HistoricalSummary ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let* block_summary_root, state_summary_root =
    ...
    | ListV [ v_block; v_state ] ->
        Ok (v_block, v_state)
    | _ -> Error (runtime at "HistoricalSummary: unexpected value shape")
  in
...
let hash_tree_root_beaconBlockBody ~at (v : Value.t) : Value.t result =
  let get_nat x = x |> Value.get_num |> Num.to_int in
  let get_list vv = match vv.it with ListV xs -> xs | _ -> [] in
  ...
  let* ( randao_reveal,
    ...
    | StructV fields ->
        ...
        if count = 11 then
          ...
          | _ -> Error (runtime at "beaconBlockBody: unexpected field count")
          ...
          | _ -> Error (runtime at "beaconBlockBody: unexpected field count")
        else Error (runtime at "beaconBlockBody: unexpected field count")
    | TupleV fields ->
        let count = List.length fields in
        if count = 11 then
          match fields with
    ...
    | _ -> Error (runtime at "beaconBlockBody: unexpected value shape")
  in
  ...
  let* r_blob_kzg_commitments =
    ...
    | Some commitments ->
        ...
        let process_kzg_commitment (commitment : Value.t) : Bytes.t result =
...
let hash_tree_root_beaconState ~at (v : Value.t) : Value.t result =
  let get_nat v = v |> Value.get_num |> Num.to_int in
  let get_list vv = match vv.it with ListV xs -> xs | _ -> [] in
  (* 28 fields *)
  let* ( genesis_time,
    ...
    | ListV
        ...
        Ok
          ( f1,
            ...
            f27,
            f28 )
    | _ -> Error (runtime at "beaconState: unexpected value shape")
  in
  ...
  let r_justification_bits =
    if List.length justification_bits_list <> 4 then
      Error (runtime at "justification_bits: must be 4 bits")
    else
...
let hash_tree_root_DepositMessage ~at (dm : Value.t) : Value.t result =
  ...
  let* pubkey_b48, wcred_b32, amount_u64 =
    ...
    | _ -> Error (runtime at "DepositMessage: unexpected value shape")
  in
  let* () = ensure_fits_bytes ~at pubkey_b48 ~len:48 in
  let* () = ensure_fits_bytes ~at wcred_b32 ~len:32 in
  let* () = ensure_fits_bytes ~at amount_u64 ~len:8 in
...
let hash_tree_root_beaconBlock ~at (blk : Value.t) : Value.t result =
  ...
  let* slot_v, proposer_v, parent_v, state_v, body_v =
    ...
    | ListV [ s; p; pr; sr; b ] ->
        Ok (s, p, pr, sr, b)
    | _ -> Error (runtime at "BeaconBlock: unexpected value shape")
  in
