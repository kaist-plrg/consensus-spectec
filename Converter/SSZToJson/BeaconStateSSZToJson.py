...
def bitfield_to_bool_list(v) -> str:
    L = v.length()
...
def main():
    #Usage : python BeaconStateSSZToJson.py --in /path/to/beaconstate_input.ssz --out /path/to/beaconstate_ouput.json

    p = argparse.ArgumentParser(description="Convert BeaconState SSZ to JSON using remerkleable types.")
    p.add_argument("--type-module", default="eth2spec.capella.mainnet", help="Python module path containing the remerkleable type (e.g., eth2spec.capella.mainnet)")
    p.add_argument("--type", dest="type_name", default="BeaconState", help="Type name inside the module (e.g., BeaconState, SignedBeaconBlock)")
